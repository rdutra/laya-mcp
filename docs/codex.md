...
Codex configuration is only a launch wrapper. The server exposes the same `info` and
`classify` tools to every MCP client and contains no Codex-specific decision policy.
